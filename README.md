...
- Las **rutas son dinámicas**: la malla se reorganiza sola. Por eso el logger de rutas va en bucle.
- **Para ver los saltos y la criticidad *cambiar* en la máquina del tiempo** hace falta que `zigbee_routes.csv` tenga **varias capturas** (varios timestamps) y que caigan **dentro del mismo periodo** que el log de RSSI (la línea de tiempo la marca el RSSI). Con una sola captura, los saltos salen constantes (es correcto). Recomendación: **lanza los dos recolectores a la vez** y deja que el de rutas complete varias rondas (cada 5 min) antes de cargar.
