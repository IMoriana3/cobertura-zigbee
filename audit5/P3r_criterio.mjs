 ...
 * El careo contra `optimal` —que decide con la MISMA función con que se cobra,
 * `poaPlantSeg` por mesa (v1.76)— lo hace audit5/P3r_careo.mjs: la distancia de
 * cada política al óptimo se parte en SOMBRA (lo que su criterio mira) y ÁNGULO
 * (lo que no mira). CONTROL: la suma de los términos reproduce `poaPlantSeg`.
 */
...
let peorRecon = 0; const t0 = Date.now(); const LZS = E.crearLazoSeg();
for (let m = 0; m < 1440; m += STEP) {
  ...
  let wt = 0, b = 0, c = 0, sg = 0, pb = 0, pc = 0;
...
const R = { pol: key, mes: mo + 1, VER, paso_min: STEP, ...acc, peor_reconstruccion_Wm2: peorRecon, s: Math.round((Date.now() - t0) / 1000), cpu: os.cpus().length };
fs.mkdirSync(path.dirname(dest), { recursive: true }); fs.writeFileSync(dest, JSON.stringify(R, null, 1));
console.log(`${key.padEnd(9)} 21-${mo === 5 ? 'jun' : 'dic'}: E ${acc.E.toFixed(6)} · haz ${acc.beam.toFixed(6)} · circ ${acc.circ.toFixed(6)} · sky+gnd ${acc.skygnd.toFixed(6)} · sombra ${(acc.perdB + acc.perdC).toFixed(6)} kWh/m² · reconstrucción ${peorRecon.toExponential(1)} · ${acc.n} pasos · ${R.s} s`);
