...
import { parcheCorrecto } from './lib_conohaz.mjs';
const ROOT = path.dirname(path.dirname(fileURLToPath(import.meta.url)));
const { F, VER } = cargaSimulador(ROOT, ['rangoHaz']);
const Fc = cargaSimulador(ROOT, ['rangoHaz'], parcheCorrecto).F;   // el cono de la fórmula del comentario
const P = cargar();
...
for (const pl of plantas) {
  const E = escena(ROOT, F, Fc, VER, P, pl);
  ...
  console.log(`${VER} · ${pl}: ${E.lineas} líneas · ${E.mesas.length} mesas · ${E.unidades.length} unidades (filas por unidad ${JSON.stringify(fil)}) · ${E.instantes.length} instantes con sol > ${P.elev_min_deg}° en ${P.dias.length} días · convergencia ${E.convergencia}° · cono ${E.cono_haz}`);
}
