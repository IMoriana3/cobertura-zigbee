...
/* 1.1 — CALIBRACIÓN DE LAS POLÍTICAS QUE ENCABEZAN LA TABLA DE E-D3.
   ...
   Ejecutable:  node audit2/D5_calibracion_plena.mjs [politicas] [pasoMin=10]
                p.ej.  node audit2/D5_calibracion_plena.mjs pairwise,optimal,optfree 10
   Salida: audit2/out/D5.txt (por stdout) + audit2/out/D5.csv                   */
import path from 'node:path'; import fs from 'node:fs';
import { spawn, execFileSync } from 'node:child_process';
import { fileURLToPath } from 'node:url';
import { EXE } from '../tools/pw_navegador.mjs';
const ROOT = path.dirname(path.dirname(fileURLToPath(import.meta.url)));
const POLS = (process.argv[2] || 'pairwise,optimal,optfree').split(',');
...
try {
  ...
  for (const key of POLS) {
    ...
    fs.writeFileSync(path.join(ROOT, 'audit2', 'out', 'D5.csv'), 'politica,kWh_m2_ano_PLENO,n_instantes,MV,nb,segundos\n' + filas.join('\n') + '\n');
  }
  console.log(`\nCSV: audit2/out/D5.csv`);
  await browser.close();
