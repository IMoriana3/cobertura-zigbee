 ...
 *     node tools/test_afbt_rayos.mjs ayora
 */
...
const r = await pg.evaluate(() => {
  ...
  function ejes(t, lado) {            // cota del EJE en las puntas [sur, norte], coordenadas de escena
    if (t.mc && t.mc.f) { const f = t.mc.f[lado]; const cv = y => (COTAS.base + y - baseElev) - COTAOFF - HOFF; return [cv(f.y[0]), cv(f.y[1])]; }
    return [t.rel + TC.postH, t.rel + TC.postH]; }
  ...
  for (const i of idx) { const A = TRK[i];
    for (const v of afbtVecinos(i)) { const B = TRK[v.j]; o.pares++;
      ...
      const bA = Math.atan(A.mc ? A.mc.gN : 0);
  ...
  return o; });
console.log(`\n${planta}: ${r.pares} parejas · con recorte en los dos ${r.ambos} · solo fórmula ${r.soloF} · solo rayos ${r.soloR} · error medio ${r.mae.toFixed(2)}° · sesgo ${r.sesgo.toFixed(2)}°`);
